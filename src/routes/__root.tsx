...
import { LayoutGrid, Wallet, LineChart, SlidersHorizontal, Menu, X, ChevronDown, ChevronUp, Plus, Mic, Send, Sparkles, Check, Loader2, Settings } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full py-8 px-4 space-y-8 bg-sidebar text-sidebar-foreground">
      <div className="flex items-center gap-3 px-2">
        ...
        <span className="text-xl font-bold tracking-tight text-sidebar-foreground">Finanças Marques</span>
      </div>
      ...
      <div className="mt-auto pt-8 border-t border-sidebar-border/50">
        <Button variant="ghost" className="w-full justify-start gap-3 px-3 text-slate-300 hover:text-white hover:bg-sidebar-accent">
...
function RootComponent() {
  ...
  const parseFinancialStatement = (text: string) => {
    ...
    if (type === "income") {
      if (lowercase.includes("salário") || lowercase.includes("salario") || lowercase.includes("empresa")) {
        category = "Renda - Salário";
      } else if (lowercase.includes("pix")) {
        category = "Renda - PIX Recebido";
      } else if (lowercase.includes("freela") || lowercase.includes("freelance") || lowercase.includes("bico")) {
        category = "Renda - Freelance";
      } else if (lowercase.includes("rendeu") || lowercase.includes("investimento") || lowercase.includes("rendimento")) {
        category = "Renda - Investimentos";
      }
    } else {
      if (lowercase.includes("água") || lowercase.includes("agua") || lowercase.includes("copasa") || lowercase.includes("saneamento")) {
        category = "Casa - Água";
      } else if (lowercase.includes("luz") || lowercase.includes("energia") || lowercase.includes("cemig")) {
        category = "Casa - Luz";
      } else if (lowercase.includes("internet") || lowercase.includes("wifi") || lowercase.includes("claro") || lowercase.includes("vivo")) {
        category = "Casa - Internet";
      } else if (lowercase.includes("mercado") || lowercase.includes("supermercado") || lowercase.includes("compra") || lowercase.includes("compras") || lowercase.includes("comida")) {
        category = "Casa - Mercado / Compras";
      } else if (lowercase.includes("gasolina") || lowercase.includes("combustivel") || lowercase.includes("combustível") || lowercase.includes("posto")) {
        category = "Moto - Gasolina";
      } else if (lowercase.includes("peça") || lowercase.includes("peças") || lowercase.includes("oficina") || lowercase.includes("pneu") || lowercase.includes("mecânico") || lowercase.includes("óleo") || lowercase.includes("oleo")) {
        category = "Moto - Peças / Manutenção";
      } else if (lowercase.includes("chatgpt") || lowercase.includes("openai") || lowercase.includes("gemini") || lowercase.includes("ia") || lowercase.includes("inteligência artificial")) {
      ...
      } else if (lowercase.includes("pix enviado") || lowercase.includes("transferi") || lowercase.includes("mandei um pix")) {
    ...
    if (type === "income") {
      description = text.replace(/(?:recebi|ganhei|salário|salario|pix|de|reais|\d+(?:[.,]\d+)?)/gi, "").trim();
      if (!description) {
      ...
      description = text.replace(/(?:gastei|comprei|paguei|reais|no|na|em|de|com|jack|rangel|\d+(?:[.,]\d+)?)/gi, "").trim();
          ...
          {isMobile && (
            <header className="h-16 border-b border-sidebar-border bg-sidebar flex items-center justify-between px-4 flex-shrink-0 z-50 relative text-sidebar-foreground w-full">
              ...
              <Button variant="ghost" size="icon" onClick={() => setMenuOpen(!menuOpen)} className="text-sidebar-foreground hover:bg-sidebar-accent">
                {menuOpen ? <X className="w-6 h-6 text-sidebar-foreground" /> : <Menu className="w-6 h-6 text-sidebar-foreground" />}
              </Button>
            </header>
          )}
            ...
            <div className="absolute top-16 left-0 w-full bg-sidebar border-b border-sidebar-border shadow-2xl flex flex-col p-4 space-y-3 z-40 max-h-[calc(100vh-8rem)] overflow-y-auto">
              ...
              <div>
                ...
                >
                  ...
                  {reportsExpanded ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
                </button>
                {reportsExpanded && (
                ...
                {settingsExpanded && (
                  <div className="pl-8 pr-3 py-2 space-y-2 bg-[#0E2C63]/40 rounded-b-lg border-x border-b border-[#0E2C63]/30 mt-1">
                    <div className="py-2 text-xs text-slate-400 border-b border-[#0E2C63]/30 pb-1">
                      Conta: <span className="font-semibold text-white">{session.user?.email || "Casal"}</span>
                    </div>
                    <Link to="/" onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-300 hover:text-white">
            ...
            >
              {aiOpen ? (
                ...
                <div className="relative w-10 h-10 flex items-center justify-center">
                  ...
                  <svg className="absolute w-9 h-9 animate-[spin_8s_linear_infinite]" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" r="40" stroke="#D4A63A" strokeWidth="4" strokeDasharray="30 30" fill="none" opacity="0.7" />
                    <circle cx="50" cy="50" r="40" stroke="#22C55E" strokeWidth="4" strokeDasharray="15 45" fill="none" opacity="0.8" />
                  </svg>
                </div>
              )}
            </button>
          ...
          {aiOpen && (
            <div className="fixed bottom-36 right-4 md:bottom-24 md:right-6 w-[calc(100vw-2rem)] sm:w-[380px] bg-white border border-slate-200 shadow-2xl rounded-2xl flex flex-col z-50 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 p-4 text-white flex justify-between items-center">
                ...
                <Button variant="ghost" size="icon" onClick={() => setAiOpen(false)} className="text-white hover:bg-white/10 rounded-full w-8 h-8">
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              <div className="flex-1 p-4 overflow-y-auto space-y-3 max-h-[300px] min-h-[220px] bg-slate-50">
                {messages.map((m) => {
                  const isPendingExpense = m.isConfirmation && pendingTransaction && pendingTransaction.type === "expense";
                  const { name: pendingResp, cleanDesc: pendingClean } = isPendingExpense 
                    ? parseResponsible(pendingTransaction.description)
                    : { name: "Os dois", cleanDesc: pendingTransaction ? pendingTransaction.description : "" };
                      ...
                      <div className={`p-3 rounded-2xl max-w-[85%] text-xs leading-relaxed ${
                          ...
                          <div className="mt-3 p-3 bg-slate-100 rounded-xl space-y-2.5 border border-slate-200">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-800">
                              <div className="space-y-0.5">
                                <span className="text-[9px] text-slate-500 font-semibold">Tipo</span>
                                <div className={`font-bold ${pendingTransaction.type === "income" ? "text-emerald-600" : "text-rose-600"}`}>
                                  {pendingTransaction.type === "income" ? "Receita (+)" : "Despesa (-)"}
                                </div>
                              </div>
                              <div className="space-y-0.5">
                                <span className="text-[9px] text-slate-500 font-semibold">Valor (R$)</span>
                                <input 
                                  type="number" 
                                  step="0.01"
                              ...
                              {pendingTransaction.type === "expense" && (
                                <div className="space-y-0.5 col-span-2">
                                  <span className="text-[9px] text-slate-500 font-semibold">Quem Gastou?</span>
                                  <select 
                                    value={pendingResp}
                                    onChange={(e) => setPendingTransaction({ 
                                      ...pendingTransaction, 
                              ...
                              <div className="space-y-0.5 col-span-2">
                                <span className="text-[9px] text-slate-500 font-semibold">Descrição</span>
                                <input 
                                  type="text" 
                                  value={pendingTransaction.type === "expense" ? pendingClean : pendingTransaction.description}
                                  onChange={(e) => setPendingTransaction({ 
                                    ...pendingTransaction, 
                                    description: pendingTransaction.type === "expense" ? `[${pendingResp}] ${e.target.value}` : e.target.value 
                                  })}
                                  className="w-full bg-white border border-slate-200 px-1.5 py-0.5 rounded font-bold outline-none text-[#0B1120] focus:ring-1 focus:ring-primary"
                                />
                              </div>
                              <div className="space-y-0.5 col-span-2">
                                <span className="text-[9px] text-slate-500 font-semibold">Categoria</span>
                                <select 
                                ...
                                >
                                  ...
                                  <option value="Renda - Investimentos">Renda - Investimentos</option>
                                  ...
                                  <option value="Casa - Mercado / Compras">Casa - Mercado / Compras</option>
                                  <option value="Moto - Gasolina">Moto - Gasolina</option>
                                  <option value="Moto - Peças / Manutenção">Moto - Peças / Manutenção</option>
                                  <option value="IA - Ferramentas (ChatGPT, Gemini...)">IA - Ferramentas</option>
                                  <option value="Cartão - Nubank">Cartão - Nubank</option>
                                  <option value="Cartão - Inter">Cartão - Inter</option>
                                  <option value="Cartão - Outro Cartão">Cartão - Outro Cartão</option>
                        ...
                        {m.isConfirmation && pendingTransaction && (
                          <div className="flex gap-2 pt-1">
                            ...
                            >
                              {aiLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3.5 h-3.5 mr-1" />}
                              Gravar na Planilha
                            </Button>
                            <Button 
                              variant="outline"
                              onClick={() => {
                                setPendingTransaction(null);
                                setMessages(prev => [...prev, { id: Date.now().toString(), type: "system", text: "Lançamento cancelado!" }]);
                              }}
              ...
              <div className="p-3 border-t border-slate-100 bg-white flex gap-2 items-center">
                <input 
                  ...
                  onKeyDown={e => { if (e.key === "Enter") handleProcessMessage(inputText); }}
