...
function SummaryCard({ title, value, icon: Icon, color, bgClass, subtitle }: { title: string; value: string; icon: any; color: string; bgClass: string; subtitle?: string }) {
  return (
...
function Index() {
    ...
    const autoMigrate = async () => {
      try {
        ...
        if (session?.user && !localStorage.getItem("migracao_final_concluida_v3")) {
          ...
          const { error: delError } = await supabase.from('transactions').delete().eq('user_id', session.user.id);
          if (delError) {
          ...
          alert("SISTEMA ATUALIZADO! Dados falsos removidos e todas as transações reais da planilha carregadas com sucesso!");
  ...
  const dividasNubank = transactions.filter(t => t.category === 'Cartão - Nubank').reduce((acc, curr) => acc + curr.amount, 0);
  const dividasInter = transactions.filter(t => t.category === 'Cartão - Inter').reduce((acc, curr) => acc + curr.amount, 0);
  ...
  const aportesReserva = transactions.filter(t => t.category === 'Renda - Investimentos').reduce((acc, curr) => acc + curr.amount, 0);
  ...
  const amortizacaoDivida = transactions.filter(t => t.description.toLowerCase().includes("quitação") || t.description.toLowerCase().includes("acordo")).reduce((acc, curr) => acc + curr.amount, 0);
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Resumo financeiro para controle de gastos e acúmulo de riqueza.</p>
        </div>
      ...
      <Card className={`border border-border/60 shadow-sm p-5 bg-white relative overflow-hidden`}>
        <div className="flex gap-4 items-start">
          <div className={`p-3 rounded-2xl ${
            tip.type === 'danger' ? 'bg-red-50 text-red-600' :
            tip.type === 'warning' ? 'bg-amber-50 text-amber-600' :
            tip.type === 'info' ? 'bg-emerald-50 text-emerald-600' : 'bg-emerald-100 text-emerald-700'
          }`}>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="border border-border/80 p-6 bg-white rounded-2xl shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right">
              <span className="font-extrabold text-lg text-[#0B1120]">{formatCurrency(totalReserva)}</span>
              <span className="text-xs text-slate-500 block">Meta: {formatCurrency(metaReserva)}</span>
            </div>
          </div>
        ...
        <Card className="border border-border/80 p-6 bg-white rounded-2xl shadow-sm">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="text-right">
              <span className="font-extrabold text-lg text-[#0B1120]">{formatCurrency(dividaRestante)}</span>
              <span className="text-xs text-slate-500 block">Dívida Inicial: {formatCurrency(metaDividaAnterior)}</span>
            </div>
          </div>
          <div className="space-y-3">
            <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
      ...
      <Card className="border border-border/80 p-6 bg-white rounded-2xl shadow-sm">
        <div className="mb-6">
          ...
          <p className="text-xs text-slate-500">Monitore o limite de gastos mensais por categoria para economizar.</p>
        </div>
          ...
          <Table>
            <TableBody>
              {transactions.slice(0, 5).map((t) => {
                ...
                return (
                  <TableRow key={t.id} className="hover:bg-slate-50 border-none transition-colors">
                    ...
                    <TableCell className={`text-right font-extrabold py-3 text-sm ${t.type === 'income' ? 'text-emerald-600' : 'text-rose-600'}`}>
