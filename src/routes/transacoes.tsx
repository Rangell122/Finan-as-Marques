...
import { Plus, Trash2, TrendingUp, TrendingDown, Filter, Calendar, DollarSign, ShoppingBag, Loader2, ArrowUpRight, ArrowDownRight, Check, AlertTriangle } from "lucide-react";
...
function TransactionsPage() {
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rawDesc = expenseLocation 
        ? `${expenseDesc} (${expenseLocation})`
        : expenseDesc;
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie contas e receitas da família com controle de vencimentos.</p>
        </div>
      ...
      <div className="relative max-w-md">
        <Input 
          placeholder={activeTab === "income" ? "Buscar por fonte ou categoria..." : "Buscar por gasto, local ou categoria..."}
      ...
      {activeTab === "income" && (
        <Card className="border border-border/80 shadow-sm bg-white p-5 rounded-2xl">
          <form onSubmit={handleAddIncome} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            >
              {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-5 h-5" />}
        ...
        <Card className="border border-border/80 shadow-sm bg-white p-5 rounded-2xl">
          <form onSubmit={handleAddExpense} className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
            <div className="space-y-1.5 md:col-span-2 lg:col-span-2">
              <span className="text-xs font-bold text-slate-500 ml-1">O que gastou? (Descrição)</span>
              <Input 
                required 
            ...
            <div className="space-y-1.5">
              <span className="text-xs font-bold text-slate-500 ml-1">Onde comprou? (Estabelecimento)</span>
              <Input 
            ...
            <div className="space-y-1.5">
              <span className="text-xs font-bold text-slate-500 ml-1">Tipo de Gasto / Categoria</span>
              <Select value={expenseCategory} onValueChange={setExpenseCategory}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Moto - Peças / Manutenção">Moto - Peças / Manutenção</SelectItem>
                  <SelectItem value="IA - Ferramentas (ChatGPT, Gemini...)">IA - Ferramentas</SelectItem>
            ...
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow className="border-b border-border">
                  ...
                  <TableHead className="font-semibold text-[#0B1120] py-4 px-6">Categoria</TableHead>
                  {activeTab === "expense" && (
                    <TableHead className="font-semibold text-[#0B1120] py-4 px-6 w-[120px]">Responsável</TableHead>
                  )}
                  <TableHead className="font-semibold text-[#0B1120] py-4 px-6 w-[120px]">Situação</TableHead>
                  <TableHead className="font-semibold text-[#0B1120] text-right py-4 px-6 w-[180px]">Valor</TableHead>
              ...
              <TableBody>
                {filteredTransactions.map((t) => {
                  ...
                  return (
                    <TableRow key={t.id} className="hover:bg-slate-50 border-b border-border transition-colors">
                      ...
                      <TableCell className="py-3.5 px-6">
                        <span className="flex items-center gap-2">
                          ...
                          <Badge variant="secondary" className="font-semibold border-none px-2.5 py-0.5 rounded-lg text-xs bg-slate-100 text-slate-700 hover:bg-slate-200">
                            {t.category}
                          </Badge>
                        </span>
                      </TableCell>
                      {activeTab === "expense" && (
                        <TableCell className="py-3.5 px-6">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold ${
                            respName === "Jack" ? "bg-violet-100 text-violet-700" :
                            respName === "Rangel" ? "bg-blue-100 text-blue-700" :
                      ...
                      <TableCell className={`text-right font-extrabold py-3.5 px-6 text-base ${
                        activeTab === "income" ? "text-emerald-600" : 
                        t.status === "Pago" ? "text-rose-600" : "text-slate-500"
                      }`}>
                ...
                {filteredTransactions.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={activeTab === "expense" ? 7 : 6} className="text-center text-slate-500 py-16">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-slate-500 flex flex-wrap gap-2 items-center pt-1">
                        <span className="flex items-center gap-1 font-medium bg-slate-100 text-slate-700 px-2 py-0.5 rounded">
                          ...
                          {t.status === "Pago" ? "Pago em: " : "Vence em: "}{formatDate(t.date)}
                        </span>
                        <Badge variant="outline" className="font-semibold text-[10px] border-slate-200 text-slate-700 px-2 py-0">
                          {t.category}
                        </Badge>
                        {activeTab === "expense" && (
      ...
      <Dialog open={!!payTransaction} onOpenChange={(open) => { if (!open) setPayTransaction(null); }}>
        <DialogContent className="sm:max-w-[400px] rounded-2xl bg-white border border-border p-6 shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-[#0B1120]">Confirmar Pagamento</DialogTitle>
          </DialogHeader>
          {payTransaction && (
            <div className="space-y-5 pt-3">
              <div className="p-3 bg-slate-100/50 rounded-xl border border-slate-200 text-sm text-[#0B1120] space-y-1">
                <div>Conta: <strong className="font-bold">{payTransaction.description}</strong></div>
                <div className="text-xs text-slate-500">Valor registrado: {formatCurrency(payTransaction.amount)}</div>
                <div className="text-xs text-slate-500">Vencimento original: {formatDate(payTransaction.date)}</div>
              </div>

              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-slate-500">Quanto você pagou de fato? (Valor R$)</Label>
                <Input 
                  required
                  type="number" 
                  step="0.01" 
